...
export function adminPage(user, tenantProfile = null) {
  const boundBusinessNotice = tenantProfile
    ...
    : "";
  const demoControls = tenantProfile
    ? '<div class="demo-controls"><button type="button" class="secondary" id="clear-business-form">Clear form for demo</button><p class="demo-mode-note" id="demo-mode-note">Clean demo form active</p></div>'
    : "";
  return `<!doctype html>
...
const boundBusinessInputs=[...form.querySelectorAll('input[readonly][name^="business"]')];const boundBusinessValues=Object.fromEntries(boundBusinessInputs.map(input=>[input.name,input.value]));const clearBusinessButton=document.querySelector('#clear-business-form');const boundBusinessNotice=document.querySelector('.bound-business');const demoModeNote=document.querySelector('#demo-mode-note');
function setCleanDemoMode(active){for(const input of boundBusinessInputs){input.value=active?'':boundBusinessValues[input.name];input.readOnly=!active}if(boundBusinessNotice)boundBusinessNotice.style.display=active?'none':'block';if(demoModeNote)demoModeNote.style.display=active?'block':'none';if(clearBusinessButton)clearBusinessButton.textContent=active?'Restore saved business':'Clear form for demo';if(active){sessionStorage.setItem('epyhia-clean-demo','1')}else{sessionStorage.removeItem('epyhia-clean-demo')}}
function matchesBoundBusiness(values){return Object.entries(boundBusinessValues).every(([field,value])=>values[field]===value)}
if(clearBusinessButton){clearBusinessButton.addEventListener('click',()=>setCleanDemoMode(boundBusinessInputs[0].readOnly));if(sessionStorage.getItem('epyhia-clean-demo')==='1')setCleanDemoMode(true)}
...
document.querySelector('#build-site').addEventListener('click',async()=>{status.textContent='Generating and reviewing the website...';try{const body=await post('/api/runs/'+encodeURIComponent(runId)+'/web-build',{},webBuildKey);const action=body.deployment.action;webBuildActionId=action.id;status.textContent='Deployment is pending your approval.\\nPayload hash: '+action.payloadHash;const approve=document.createElement('button');approve.type='button';approve.textContent='Approve exact payload and deploy';approve.addEventListener('click',async()=>{approve.disabled=true;status.textContent='Deploying the approved payload and verifying its URL...';try{const deployed=await post('/api/actions/'+encodeURIComponent(action.id)+'/approve-and-execute',{payloadHash:action.payloadHash});status.textContent='Verified live site: '+deployed.execution.deployment.liveUrl+'\\nComplete one test purchase, then use the idempotency proof below.';proofDashboard.style.display='block';refreshTaskStatus()}catch(error){status.textContent=error.message;approve.disabled=false}});status.append(document.createElement('br'),approve)}catch(error){status.textContent=error.message}});
