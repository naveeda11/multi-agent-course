...
function showWebsitePreview(build){currentSiteAction=build.deployment.action;webBuildActionId=currentSiteAction.id;siteFrame.srcdoc=build.draft.html;siteHash.textContent='Exact deployment payload hash: '+currentSiteAction.payloadHash;sitePreview.style.display='block';recoverWebsite.hidden=true;approveSite.disabled=currentSiteAction.status==='EXECUTED';approveSite.textContent=currentSiteAction.status==='EXECUTED'?'Website deployed':'Approve website and go live'}
