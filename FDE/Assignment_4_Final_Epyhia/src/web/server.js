...
form.addEventListener('submit',async(event)=>{event.preventDefault();status.style.display='block';status.textContent='Creating a traceable run...';actions.style.display='none';marketingPreview.style.display='none';proofDashboard.style.display='none';clarifications.replaceChildren();try{const values=Object.fromEntries(new FormData(form));onboardingPayload={...values,approvedBudgetMicrodollars:Math.round(Number(values.budgetDollars)*1000000)};onboardingKey=crypto.randomUUID();clarificationRound=0;clarificationHistory=[];completeOnboarding(await post('/api/onboarding',onboardingPayload,onboardingKey))}catch(error){status.textContent=error.message}});
