...
export function adminPage(user, tenantProfile = null) {
  ...
  const deletionControls = tenantProfile
    ? '<section class="danger-zone" aria-labelledby="delete-business-heading"><h3 id="delete-business-heading">Delete business and undeploy</h3><p>Permanently removes this tenant’s deployed site, R2 artifacts, Stripe test associations, EPYHIA database records, and Auth0 identity.</p><button type="button" id="delete-business">Delete everything</button><p id="deletion-status" aria-live="polite"></p></section>'
    : "";
  return `<!doctype html>
...
const deleteBusinessButton=document.querySelector('#delete-business');const deletionStatus=document.querySelector('#deletion-status');if(deleteBusinessButton){deleteBusinessButton.addEventListener('click',async()=>{if(!confirm('Delete this business, undeploy its site, erase its provider artifacts, orders, audit history, and Auth0 identity? This cannot be undone.'))return;deleteBusinessButton.disabled=true;deletionStatus.textContent='Deleting provider data, undeploying the site, and erasing the tenant...';try{const response=await fetch('/api/tenant',{method:'DELETE',headers:{'content-type':'application/json'},body:JSON.stringify({confirmation:'DELETE'})});const result=await response.json();if(!response.ok)throw new Error(result.error?.message||'Deletion failed.');location.href='/logout'}catch(error){deletionStatus.textContent=error.message;deleteBusinessButton.disabled=false}})}
