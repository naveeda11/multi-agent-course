...
function showWebsitePreview(build){currentSiteAction=build.deployment.action;webBuildActionId=currentSiteAction.id;siteFrame.srcdoc=build.draft.html;siteHash.textContent='Exact deployment payload hash: '+currentSiteAction.payloadHash;sitePreview.style.display='block';approveSite.disabled=currentSiteAction.status==='EXECUTED';approveSite.textContent=currentSiteAction.status==='EXECUTED'?'Website deployed':'Approve website and go live'}
...
async function restorePersistedDeliverables(latest){const deliverables=await fetchRunDeliverables();if(deliverables.website){websiteGenerationState.textContent='Ready for your deployment review';showWebsitePreview(deliverables.website)}else{const webTask=latest?.tasks.find(task=>task.taskType==='WEB_BUILD');websiteGenerationState.textContent=webTask?.status==='PENDING'?'Generation is not complete':webTask?.status||'No website output'}if(deliverables.marketing){marketingGenerationState.textContent='Ready for your marketing review';showMarketingPreview(deliverables.marketing.pack,deliverables.marketing.persisted)}else{const marketingTask=latest?.tasks.find(task=>task.taskType==='MARKETING_PACK');marketingGenerationState.textContent=marketingTask?.status==='PENDING'?'Generation is not complete':marketingTask?.status||'No marketing output'}const ready=Number(Boolean(deliverables.website))+Number(Boolean(deliverables.marketing));if(ready===2)status.textContent='Website and marketing pack restored. Review and approve each one independently.';else if(ready===1)status.textContent='One completed deliverable has been restored and is ready for your approval.';else status.textContent='No completed deliverable is available to review yet.';return deliverables}
