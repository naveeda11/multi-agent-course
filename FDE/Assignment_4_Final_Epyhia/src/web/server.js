...
@media(max-width:850px){.document-grid,.generation-grid{grid-template-columns:1fr}.site-frame{height:520px}}
...
function showVideoApproval(action){currentVideoAction=action;const approval=document.createElement('div');approval.className='review-approval';const estimate=document.createElement('p');estimate.textContent='Marketing pack approved. Optional next action: $0.64 for two 4-second Fast video outputs.';const hashLabel=document.createElement('p');hashLabel.className='hash';hashLabel.textContent='Exact video payload hash: '+action.payloadHash;const approve=document.createElement('button');approve.type='button';approve.textContent='Approve $0.64 video render';approve.addEventListener('click',async()=>{approve.disabled=true;status.textContent='Rendering the exact approved 16:9 and 9:16 video payloads...';try{const rendered=await post('/api/actions/'+encodeURIComponent(action.id)+'/approve-and-render',{payloadHash:action.payloadHash});status.textContent='Two video artifacts are stored in R2. Logged cost: $'+(rendered.execution.action.providerCostMicrodollars/1000000).toFixed(2);refreshTaskStatus()}catch(error){status.textContent=error.message;approve.disabled=false}});approval.append(estimate,hashLabel,approve);marketingContent.append(approval)}
function showMarketingPreview(pack,persisted){currentPackHash=persisted.packHash;currentVideoAction=persisted.videoAction;marketingContent.replaceChildren();marketingContent.append(reviewSection('Landing copy',pack.landingCopy));for(const [index,post] of pack.socialPosts.entries()){marketingContent.append(reviewSection('Social post '+(index+1)+' ('+post.channel+')',post.text))}marketingContent.append(reviewSection('Launch email draft',pack.launchEmail));marketingContent.append(reviewSection('Storyboard summary',pack.storyboard.summary));marketingContent.append(reviewSection('Landscape video prompt',pack.storyboard.landscapePrompt));marketingContent.append(reviewSection('Vertical video prompt',pack.storyboard.verticalPrompt));const approval=document.createElement('div');approval.className='review-approval';const hashLabel=document.createElement('p');hashLabel.className='hash';hashLabel.textContent='Exact marketing-pack hash: '+persisted.packHash;const approve=document.createElement('button');approve.type='button';approve.textContent='Approve marketing pack';approve.addEventListener('click',async()=>{approve.disabled=true;status.textContent='Recording approval for the exact marketing pack...';try{const result=await post('/api/runs/'+encodeURIComponent(runId)+'/marketing-pack/approve',{packHash:persisted.packHash});approval.replaceChildren();const complete=document.createElement('p');complete.className='approval-complete';complete.textContent='Marketing pack approved. The paid video action remains separate.';approval.append(complete);showVideoApproval(result.videoAction);status.textContent='Marketing pack approved. Review the separate video cost before rendering.';refreshTaskStatus()}catch(error){status.textContent=error.message;approve.disabled=false}});approval.append(hashLabel,approve);marketingContent.append(approval);marketingPreview.style.display='block'}
function showWebsitePreview(build){currentSiteAction=build.deployment.action;webBuildActionId=currentSiteAction.id;siteFrame.srcdoc=build.draft.html;siteHash.textContent='Exact deployment payload hash: '+currentSiteAction.payloadHash;sitePreview.style.display='block';approveSite.disabled=false;approveSite.textContent='Approve website and go live'}
function showBrandDocument(context){currentBrandDocument=context.brandDocument;completedBrief.textContent=context.completedBrief;brandDocument.textContent=context.brandDocument.fullText;brandHash.textContent='Brand version '+context.brandDocument.version+' · exact content hash '+context.brandDocument.contentHash;brandReview.style.display='block';generationPanel.style.display='block';approveBrand.disabled=context.brandDocument.approvalStatus==='APPROVED';if(context.brandDocument.approvalStatus==='APPROVED'){approveBrand.textContent='Brand approved';websiteGenerationState.textContent='Approval recorded';marketingGenerationState.textContent='Approval recorded'}else{approveBrand.textContent='Approve brand and generate';websiteGenerationState.textContent='Waiting for brand approval';marketingGenerationState.textContent='Waiting for brand approval'}}
async function runAutomaticGeneration(){approveBrand.disabled=true;generationPanel.style.display='block';websiteGenerationState.textContent='Generating and reviewing...';marketingGenerationState.textContent='Generating and grounding...';status.textContent='Brand approved. Web Builder and Marketer are now running automatically...';try{const result=await post('/api/runs/'+encodeURIComponent(runId)+'/brand-document/approve-and-generate',{brandDocumentId:currentBrandDocument.id,contentHash:currentBrandDocument.contentHash});approveBrand.textContent='Brand approved';if(result.generation.website.status==='COMPLETED'){websiteGenerationState.textContent='Ready for your deployment review';showWebsitePreview(result.generation.website.value)}else{websiteGenerationState.textContent='Failed: '+result.generation.website.error.message}if(result.generation.marketing.status==='COMPLETED'){marketingGenerationState.textContent='Ready for your marketing review';showMarketingPreview(result.generation.marketing.value.pack,result.generation.marketing.value.persisted)}else{marketingGenerationState.textContent='Failed: '+result.generation.marketing.error.message}status.textContent='Automatic generation finished. Review each completed deliverable below before approving an external action.';refreshTaskStatus()}catch(error){status.textContent=error.message;approveBrand.disabled=false}}
approveBrand.addEventListener('click',runAutomaticGeneration);
approveSite.addEventListener('click',async()=>{if(!currentSiteAction)return;approveSite.disabled=true;status.textContent='Deploying the exact reviewed website and verifying its URL...';try{const deployed=await post('/api/actions/'+encodeURIComponent(currentSiteAction.id)+'/approve-and-execute',{payloadHash:currentSiteAction.payloadHash});approveSite.textContent='Website deployed';status.textContent='Verified live site: '+deployed.execution.deployment.liveUrl+'\\nComplete one test purchase, then use the idempotency proof below.';proofDashboard.style.display='block';refreshTaskStatus()}catch(error){status.textContent=error.message;approveSite.disabled=false}});
